...
class MainWindow(QMainWindow):
    ...
    def on_analysis_completed(self, step_name, result):
        ...
        print(f"结果类型: step_name={type(step_name)}, result={type(result)}")
        
        if result:
            # 更新分析状态
            self.analysis_status_label.setText("✅ 分析完成")
            
            # 完全解析QwenAI返回的JSON到对应变量
            try:
                # 如果result是步骤名称字符串，跳过处理
                if isinstance(result, str) and result in ["上左", "上中", "上右", "下左", "下中", "下右", "综合评价"]:
                    print(f"跳过步骤名称: {result}")
                    return
                
                parsed_data = self._parse_qwen_result(result)
                score = parsed_data['score']
                is_correct = parsed_data['is_correct']
                feedback = parsed_data['feedback']
                detected_issues = parsed_data['detected_issues']
                good_points = parsed_data['good_points']
                
                print(f"解析成功 - 得分:{score}, 正确:{is_correct}, 反馈长度:{len(feedback)}")
            ...
            if is_correct and score >= 80:
                status_text = f"✅ 动作规范 (得分: {score})"
            elif is_correct and score >= 60:
                status_text = f"✅ 基本正确 (得分: {score})"
            elif score > 0:
                status_text = f"⚠️ 需要改进 (得分: {score})"
            else:
                status_text = f"❌ 未检测到刷牙 (得分: {score})"
            ...
            if isinstance(result, str):
                print(f"原始结果字符串: {result[:500]}...")
                
                # 尝试提取嵌套JSON中的feedback
                import re
                
                # 方法1: 查找```json块中的feedback
                json_block_match = re.search(r'```json\s*\n\s*\{\s*.*?"feedback":\s*"([^"]*)"', result, re.DOTALL)
                if json_block_match:
                    detailed_feedback_text = json_block_match.group(1)
                    print(f"从JSON块提取到feedback: {detailed_feedback_text}")
                else:
                    # 方法2: 查找任何位置的"feedback": "内容"
                    feedback_match = re.search(r'"feedback":\s*"([^"]*)"', result)
                    if feedback_match:
                        detailed_feedback_text = feedback_match.group(1)
                        print(f"从字符串提取到feedback: {detailed_feedback_text}")
                    else:
                        # 方法3: 如果包含"图片中没有"等关键词，直接提取这部分
                        if "图片中没有" in result:
                            # 查找包含"图片中没有"的句子
                            sentences = result.split('。')
                            for sentence in sentences:
                                if "图片中没有" in sentence:
                                    # 清理句子，移除多余的符号和空格
                                    clean_sentence = sentence.strip().replace('"', '').replace(',', '')
                                    if clean_sentence:
                                        detailed_feedback_text = clean_sentence + "。"
                                        break
                        
                        if not detailed_feedback_text:
                            detailed_feedback_text = feedback
            else:
                detailed_feedback_text = feedback
            
            # 如果还是没有有效内容，显示解析后的信息
            if not detailed_feedback_text or detailed_feedback_text == "动作分析完成":
                if detected_issues:
                    detailed_feedback_text = "🔍 发现问题:\n" + "\n".join([f"• {issue}" for issue in detected_issues[:3]])
                elif score == 0:
                    detailed_feedback_text = "图片中没有看到牙刷或刷牙动作，无法评估刷牙规范性"
                else:
                    detailed_feedback_text = f"动作分析完成，得分: {score}分"
            
            print(f"最终显示的详细反馈: {detailed_feedback_text}")
            self.detailed_feedback_label.setText(detailed_feedback_text)
            
        else:
            self.analysis_status_label.setText("❌ 分析失败")
            self.feedback_label.setText("分析服务暂时不可用")
    
    def _parse_qwen_result(self, result):
        """完全解析QwenAI返回的JSON结果"""
        # 初始化默认值
        parsed_data = {
            'score': 0,
        ...
        try:
            print(f"开始解析QwenAI结果: {str(result)[:200]}...")
            
            if isinstance(result, dict):
                # 直接是字典类型，直接提取数据
                parsed_data['score'] = self._safe_get_int(result.get('score', 0))
                parsed_data['is_correct'] = bool(result.get('is_correct', False))
                parsed_data['feedback'] = self._clean_feedback_text(result.get('feedback', '动作分析完成'))
                parsed_data['detected_issues'] = self._safe_get_list(result.get('detected_issues', []))
                parsed_data['good_points'] = self._safe_get_list(result.get('good_points', []))
                ...
                try:
                    ...
                    print("直接JSON解析失败，尝试修复和提取JSON部分")
                    ...
                    if json_match:
                        ...
                        print(f"提取到JSON字符串: {json_str[:200]}...")
                        
                        # 修复常见的JSON语法错误
                        # 1. 在字段之间添加缺失的逗号 - 更精确的匹配
                        json_str = re.sub(r'"\s*\n\s*"', '",\n    "', json_str)
                        json_str = re.sub(r'"\s*\n\s*\[', '",\n    [', json_str)
                        json_str = re.sub(r'\]\s*\n\s*"', '],\n    "', json_str)
                        ...
                        # 2. 修复数组格式 - 处理没有逗号的数组元素
                        json_str = re.sub(r'\[\s*"([^"]*?)"\s+"([^"]*?)"\s*\]', r'["\1", "\2"]', json_str)
                        json_str = re.sub(r'\[\s*"([^"]*?)"\s+"([^"]*?)"\s+"([^"]*?)"\s*\]', r'["\1", "\2", "\3"]', json_str)
                        
                        # 3. 处理特殊情况：字段值后直接跟字段名
                        json_str = re.sub(r'(:\s*"[^"]*")\s+("[\w_]+":)', r'\1,\n    \2', json_str)
                        json_str = re.sub(r'(:\s*\d+)\s+("[\w_]+":)', r'\1,\n    \2', json_str)
                        json_str = re.sub(r'(:\s*(true|false))\s+("[\w_]+":)', r'\1,\n    \3', json_str)
                        json_str = re.sub(r'(:\s*\[[^\]]*\])\s+("[\w_]+":)', r'\1,\n    \2', json_str)
                        
                        print(f"修复后JSON字符串: {json_str[:200]}...")
                        
                        try:
                            json_data = json.loads(json_str)
                            print("JSON修复和解析成功")
                        except json.JSONDecodeError as e:
                            print(f"JSON修复后仍解析失败: {e}")
                            print(f"完整修复后JSON: {json_str}")
                            ...
                            if score_match:
                                json_data['score'] = int(score_match.group(1))
                            
                            # 提取is_correct
                            correct_match = re.search(r'"is_correct":\s*(true|false)', json_str)
                            if correct_match:
                                json_data['is_correct'] = correct_match.group(1) == 'true'
                            
                            # 提取feedback - 支持多行内容
                            feedback_match = re.search(r'"feedback":\s*"([^"]*(?:\\.[^"]*)*)"', json_str, re.DOTALL)
                            if feedback_match:
                                feedback_text = feedback_match.group(1)
                                # 处理转义字符
                                feedback_text = feedback_text.replace('\\n', ' ').replace('\\"', '"')
                                json_data['feedback'] = feedback_text
                            
                            # 提取detected_issues数组
                            issues_match = re.search(r'"detected_issues":\s*\[([^\]]*)\]', json_str)
                            if issues_match:
                                issues_str = issues_match.group(1)
                                issues = re.findall(r'"([^"]*)"', issues_str)
                                json_data['detected_issues'] = issues
                            
                            # 提取good_points数组
                            points_match = re.search(r'"good_points":\s*\[([^\]]*)\]', json_str)
                            if points_match:
                                points_str = points_match.group(1)
                                points = re.findall(r'"([^"]*)"', points_str)
                                json_data['good_points'] = points
                            
                            print(f"手动提取的JSON数据: {json_data}")
                
                if json_data and isinstance(json_data, dict):
                    print(f"JSON数据: {json_data}")
                    parsed_data['score'] = self._safe_get_int(json_data.get('score', 0))
                    parsed_data['is_correct'] = bool(json_data.get('is_correct', False))
                    parsed_data['feedback'] = self._clean_feedback_text(json_data.get('feedback', '动作分析完成'))
                    parsed_data['detected_issues'] = self._safe_get_list(json_data.get('detected_issues', []))
                    parsed_data['good_points'] = self._safe_get_list(json_data.get('good_points', []))
                else:
                    # 无法解析JSON，检查是否包含有用信息
                    if any(keyword in result.lower() for keyword in ['score', 'feedback', 'correct', '得分', '反馈']):
                        parsed_data['feedback'] = self._clean_feedback_text(result)
                    else:
                        parsed_data['feedback'] = "无法解析分析结果"
                    
        except Exception as e:
            print(f"QwenAI结果解析错误: {e}")
            parsed_data['feedback'] = "分析结果解析出错"
        
        print(f"最终解析结果: score={parsed_data['score']}, is_correct={parsed_data['is_correct']}, feedback='{parsed_data['feedback'][:50]}...'")
        return parsed_data
    
    def _safe_get_int(self, value):
        """安全获取整数值"""
        try:
    ...
    def _safe_get_list(self, value):
        """安全获取列表值"""
        if isinstance(value, list):
            return [str(item) for item in value if item]  # 确保列表项都是字符串
    ...
    def _clean_feedback_text(self, text):
        ...
        return cleaned if cleaned else "动作分析完成"
